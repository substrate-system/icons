...
export const render = createRenderer(createHtml, 'new-tab')

function createHtml ({ title }: { title?: string } = {}) {
    if (!title) title = 'New Tab'
    const kebabTitle = kebabCase(title)

    return `<svg role="image" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
        class="icon new-tab"
...
export function outerHtml ({ title }: { title?: string }) {
    return `<new-tab>${render({ title })}</new-tab>`
}
